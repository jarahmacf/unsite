...
export type Snapshot={schema_version:"2.0"|"2.1";id:string;name:string;kind:"person"|"business"|"project"|"collection";description:string;contact_email?:string|null;records:PublishedRecord[]};
...
export function releaseDiff(before:Snapshot|undefined,after:Snapshot){
  ...
  const profileChanged=!before||["name","kind","description","contact_email"].some(k=>String(before[k as keyof Snapshot]||"")!==String(after[k as keyof Snapshot]||""));
  return {added,changed,removed,profileChanged};
}
