...
export type Segment={id:string;space_id:string;source_version_id:string;ordinal:number;start_char:number;end_char:number;text:string;locator:string};
...
export type FrozenRecord={id:string;revision:number;title:string;text:string;kind:RecordKind;fields:KnowledgeFields;context:KnowledgeContext;links?:unknown[]};
export type CollectionBatch={source_items:string[];record_ids:string[]};
...
const words=(s:string)=>new Set(s.normalize("NFKC").toLowerCase().match(/[\p{L}\p{N}]{3,}/gu)||[]);
...
export function planCollection(items:SourceItem[],segments:Segment[],records:FrozenRecord[]):CollectionBatch[]{
  ...
  while(remaining.length){
    ...
    const terms=words(selected.map(i=>[i.title,...i.context.aliases,...i.context.topics].join(" ")).join(" "));
    let recordSize=0;
    const related=records.map(r=>({r,score:overlap(terms,labels(r))})).filter(x=>x.score>0).sort((a,b)=>b.score-a.score||a.r.id.localeCompare(b.r.id)).filter(({r})=>{const n=JSON.stringify(r).length;if(recordSize+n>55000)return false;recordSize+=n;return true;}).slice(0,4);
    batches.push({source_items:selected.map(i=>i.id),record_ids:related.map(x=>x.r.id)});
  }
  return batches;
}
