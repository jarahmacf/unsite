"use client";
import {Children,cloneElement,createContext,isValidElement,useContext,useId,useRef,useState,type ReactNode} from "react";
import {Loader2,ArrowUpRight,ChevronDown,Check,X} from "@/components/unsite/icons";
import {Button} from "@/components/ui/button";
import {Select,SelectContent,SelectItem,SelectTrigger,SelectValue} from "@/components/ui/select";
import {Sheet,SheetContent,SheetHeader,SheetTitle,SheetDescription} from "@/components/ui/sheet";
export class RequestError extends Error { constructor(message:string,public status:number){super(message);this.name="RequestError";} }
export async function api<T=Record<string,unknown>>(path:string,body?:unknown):Promise<T>{
  let r:Response;
  try{r=await fetch(path,{method:body===undefined?"GET":"POST",credentials:"same-origin",cache:"no-store",signal:AbortSignal.timeout(30000),headers:body===undefined?{}:{"Content-Type":"application/json"},...(body===undefined?{}:{body:JSON.stringify(body)})});}
  catch{throw new RequestError("The connection was interrupted. Your saved work is safe; try again.",0);}
  let data;try{data=await r.json();}catch{throw new Error("The connection was interrupted. Please try again.");}
  if(!r.ok)throw new RequestError(data.error||"The request could not be completed.",r.status);return data;
}
...
export function Choice({value,onChange,items,label,disabled}:{value:string;onChange:(s:string)=>void;items:{value:string;label:string}[];label:string;disabled?:boolean}){
  return <Select value={value} onValueChange={onChange} disabled={disabled}><SelectTrigger className="us-select" aria-label={label}><SelectValue/><ChevronDown size={13}/></SelectTrigger><SelectContent className="us-select-menu">{items.map(i=><SelectItem key={i.value} value={i.value}><span className="us-select-label">{i.label}</span>{i.value===value&&<Check size={13} className="us-select-check"/>}</SelectItem>)}</SelectContent></Select>;
}
...
export function Modal({open,onClose,title,description,children,wide=false,dirty,readOnly=false}:{open:boolean;onClose:()=>void;title:string;description:string;children:ReactNode;wide?:boolean;dirty?:boolean;readOnly?:boolean}){
  const restoreFocus=useRef<HTMLElement|null>(null);
  const [touched,setTouched]=useState(false),[discard,setDiscard]=useState(false);
  function dismiss(){if(!readOnly&&(dirty??touched)){setDiscard(true);return;}onClose();}
  return <Sheet open={open} onOpenChange={v=>!v&&dismiss()}><SheetContent side="right" showCloseButton={false} className={"us-dialog us-panel "+(wide?"us-panel-wide":"")} onOpenAutoFocus={()=>{restoreFocus.current=document.activeElement instanceof HTMLElement?document.activeElement:null;}} onCloseAutoFocus={event=>{event.preventDefault();if(restoreFocus.current?.isConnected)restoreFocus.current.focus();else document.getElementById("workspace-content")?.focus();}}><SheetHeader className="us-panel-header"><SheetTitle>{title}</SheetTitle><SheetDescription>{description}</SheetDescription></SheetHeader><button type="button" className="us-icon-button us-panel-close" aria-label={"Close "+title} onClick={dismiss}><X size={16}/></button><div className="us-panel-body" onChangeCapture={()=>setTouched(true)}>{children}</div>{discard&&<div className="us-panel-discard" role="alert"><p>You have unsaved changes. Keep editing or discard them?</p><div><Action secondary onClick={()=>setDiscard(false)}>Keep editing</Action><Action onClick={()=>{setDiscard(false);onClose();}}>Discard changes</Action></div></div>}</SheetContent></Sheet>;
}
